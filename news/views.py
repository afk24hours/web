...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            mail = send_mail(form.cleaned_data['subject'],form.cleaned_data['content'], 'tdjango@yandex.ru', ['sinister.kz@gmail.com',], fail_silently=True)
            if mail:
                messages.success(request,'Письмо было отправлено.')
